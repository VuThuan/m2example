...
use \Bdcrops\Afterpay\Model\Adapter\V1\AfterpayOrderTokenV1 as AfterpayOrderTokenV1;
...
class Process extends \Magento\Framework\App\Action\Action
{
    ...
    public function _processAuthorizeCapture()
    {
        ...
        $payment->setMethod(\Bdcrops\Afterpay\Model\Payovertime::METHOD_CODE);
    ...
    private function _getAfterPayOrderToken($afterpayOrderToken, $payment, $targetObject)
    {
        if ($targetObject && $targetObject->getReservedOrderId()) {
            $result = $afterpayOrderToken->generate($targetObject, \Bdcrops\Afterpay\Model\Payovertime::AFTERPAY_PAYMENT_TYPE_CODE_V1, ['merchantOrderId' => $targetObject->getReservedOrderId() ]);
        } elseif ($targetObject) {
            $result = $afterpayOrderToken->generate($targetObject, \Bdcrops\Afterpay\Model\Payovertime::AFTERPAY_PAYMENT_TYPE_CODE_V1);
        }
        ...
        if ($orderToken) {
            $payment->setAdditionalInformation(\Bdcrops\Afterpay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_TOKEN, $orderToken);
        } else {
