 ...
 */
namespace Bdcrops\Afterpay\Controller\Payment;

use \Magento\Payment\Model\Method\AbstractMethod;

/**
 * Class Response
 * @package Bdcrops\Afterpay\Controller\Payment
 */
class Response extends \Magento\Framework\App\Action\Action
{
     ...
     * @param \Bdcrops\Afterpay\Model\Response $response
     */
    public function __construct(
        \Magento\Framework\App\Action\Context $context,
        \Magento\Framework\Controller\Result\ForwardFactory $resultForwardFactory,
        \Magento\Checkout\Model\Session $checkoutSession,
        \Bdcrops\Afterpay\Model\Response $response,
        \Bdcrops\Afterpay\Helper\Data $helper,
        \Bdcrops\Afterpay\Model\Adapter\V1\AfterpayOrderDirectCapture $directCapture,
        \Bdcrops\Afterpay\Model\Adapter\V1\AfterpayOrderTokenCheck $tokenCheck,
        \Magento\Framework\Json\Helper\Data $jsonHelper,
        \Bdcrops\Afterpay\Model\Config\Payovertime $afterpayConfig,
    ...
    {
        // debug mode
        $this->_helper->debug('Start \Bdcrops\Afterpay\Controller\Payment\Response::execute() with request ' . $this->_jsonHelper->jsonEncode($this->getRequest()->getParams()));
        ...
        // debug mode
        $this->_helper->debug('Finished \Bdcrops\Afterpay\Controller\Payment\Response::execute()');
    ...
    private function _processAuthCapture($query)
    {
        ...
        try {
            switch ($query['status']) {
                case \Bdcrops\Afterpay\Model\Response::RESPONSE_STATUS_CANCELLED:
                    $this->messageManager->addError(__('You have cancelled your Afterpay payment. Please select an alternative payment method.'));
                    break;
                case \Bdcrops\Afterpay\Model\Response::RESPONSE_STATUS_FAILURE:
                    $this->messageManager->addError(__('Afterpay payment failure. Please select an alternative payment method.'));
                    break;
                case \Bdcrops\Afterpay\Model\Response::RESPONSE_STATUS_SUCCESS:
                    //Steven - Capture here
                    $quote = $this->_checkoutSession->getQuote();

                    $payment = $quote->getPayment();

                    $token = $payment->getAdditionalInformation(\Bdcrops\Afterpay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_TOKEN);
                    ...
                    if (empty($response['status'])) {
                        $response['status'] = \Bdcrops\Afterpay\Model\Response::RESPONSE_STATUS_DECLINED;
                        $this->_helper->debug("Transaction Exception (Empty Response): " . json_encode($response));
                    }

                    switch ($response['status']) {
                        case \Bdcrops\Afterpay\Model\Response::RESPONSE_STATUS_APPROVED:
                            $payment->setAdditionalInformation(\Bdcrops\Afterpay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_ORDERID, $response['id']);
                            ...
                            break;
                        case \Bdcrops\Afterpay\Model\Response::RESPONSE_STATUS_DECLINED:
                            $this->messageManager->addError(__('Afterpay payment declined. Please select an alternative payment method.'));
                            break;
                        // case \Bdcrops\Afterpay\Model\Response::RESPONSE_STATUS_PENDING:
