...
class Response
{
     ...
     * @param \Bdcrops\Afterpay\Model\Adapter\AfterpayPayment $afterpayApiPayment
     * @param \Bdcrops\Afterpay\Helper\Data $helper
    ...
    public function __construct(
        ...
        \Bdcrops\Afterpay\Model\Adapter\AfterpayPayment $afterpayApiPayment,
    ...
    public function validCallback(\Magento\Sales\Model\Order $order, $response = [])
    {
        ...
        if ($this->request->getParam('orderToken') ===  $additionalInfo[\Bdcrops\Afterpay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_TOKEN]) {
            return true;
        }
        return false;
    }
    ...
    public function processSuccessPayment(\Magento\Sales\Model\Order $order, $orderId)
    {
        ...
        if ($order->canInvoice() && $this->_shouldInvoiced($order, $orderId)) {
            ...
            switch ($this->status) {
                case \Bdcrops\Afterpay\Model\Status::STATUS_APPROVED:
                    // create invoice and update order
                    $this->createInvoiceAndUpdateOrder($order, $orderId);
                    break;
                case \Bdcrops\Afterpay\Model\Status::STATUS_PENDING:
    ...
    public function updatePayment(\Magento\Sales\Model\Order $order, $orderId)
    {
        ...
        $payment->setAdditionalInformation(\Bdcrops\Afterpay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_ORDERID, $orderId);
    ...
    protected function _shouldInvoiced(\Magento\Sales\Model\Order $order, $afterpayOrderId)
    {
        ...
        if ($order->getPayment()->getMethod() !== \Bdcrops\Afterpay\Model\Payovertime::METHOD_CODE) {
            return false;
        }
        ...
        if ($response['token'] == $order->getPayment()->getAdditionalInformation(\Bdcrops\Afterpay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_TOKEN) &&
            ($response['status'] == \Bdcrops\Afterpay\Model\Status::STATUS_APPROVED || $response['status'] == \Bdcrops\Afterpay\Model\Status::STATUS_PENDING)
        ) {
            return true;
        }
