...
use \Bdcrops\Afterpay\Helper\Data as Helper;
...
class Payovertime extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    protected $_infoBlockType = 'Bdcrops\Afterpay\Block\Info';
    ...
    public function __construct(
        ...
        \Bdcrops\Afterpay\Model\Adapter\V1\AfterpayOrderTokenV1 $afterpayOrderTokenV1,
        \Bdcrops\Afterpay\Model\Adapter\AfterpayPayment $afterpayPayment,
        \Bdcrops\Afterpay\Model\Response $afterpayResponse,
    ...
    public function capture(InfoInterface $payment, $amount)
    {
        ...
        $token_generated = $payment->getAdditionalInformation(\Bdcrops\Afterpay\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_TOKENGENERATED);
    ...
    {
        // debug mode
        $this->helper->debug('Start \Bdcrops\Afterpay\Model\Payovertime::refund()');
        ...
        if ($orderId) {
            ...
            if (!empty($response['refundId'])) {
                // debug mode
                $this->helper->debug('Finished \Bdcrops\Afterpay\Model\Payovertime::refund()');
                return $this;
            } else {
    ...
    {
        // Debug mode
        $this->helper->debug('Start \Bdcrops\Afterpay\Model\Payovertime::fetchTransactionInfo()');
        ...
        if ($orderScopeDate < $requestDate) {
            ...
            if (isset($response['totalResults']) && $response['totalResults'] > 0) {
                ...
                switch ($result['status']) {
                    ...
                    case \Bdcrops\Afterpay\Model\Status::STATUS_DECLINED;
                    ...
                    case \Bdcrops\Afterpay\Model\Status::STATUS_FAILED;
                        // set payment denied and will canceled the order
        ...
        // Debug mode
        $this->helper->debug('Finished \Bdcrops\Afterpay\Model\Payovertime::fetchTransactionInfo()');
